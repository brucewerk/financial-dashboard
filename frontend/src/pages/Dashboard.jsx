...
const Dashboard = () => {
  ...
  const loadData = async () => {
    try {
      ...
      setStats({
        ...
        monthlyVariation: statsData.monthlyVariation || 0,
        variationPercent: statsData.variationPercent || 0,
      });
  ...
  // Dados processados com useMemo para performance
  const balanceData = useMemo(
    () =>
      monthlyBalances.slice(-12).map((b) => ({
  ...
  const investmentData = useMemo(
    () =>
      Object.entries(investmentByType).map(([name, value]) => ({
        name,
        value,
      })),
    [investmentByType],
  ...
  const StatCard = useCallback(
    ...
    [isMobile, formatCurrency, theme.shadows],
  );
  ...
  return (
    <Box>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        >
          ...
          <Box
            sx={{
              width: "100%",
              height: isMobile ? 280 : isTablet ? 350 : 420,
            }}
          >
      ...
      {/* GRÁFICO 3 - Ativos vs Passivos por Mês */}
      <Box sx={{ mb: 4 }}>
        <Paper
          sx={{
            p: { xs: 2, sm: 3 },
        ...
        >
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                <defs>
                  <linearGradient id="barAtivos" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#1976d2" stopOpacity={0.9} />
                    <stop offset="95%" stopColor="#1976d2" stopOpacity={0.6} />
                  </linearGradient>
                  <linearGradient id="barPassivos" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#dc004e" stopOpacity={0.9} />
                    <stop offset="95%" stopColor="#dc004e" stopOpacity={0.6} />
                  </linearGradient>
                </defs>
                <CartesianGrid
                  strokeDasharray="3 3"
      ...
      <Box sx={{ mb: 2 }}>
        ...
        >
          ...
          <Box
            sx={{
              width: "100%",
              height: isMobile ? 250 : isTablet ? 300 : 380,
            }}
          >
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
              ...
              >
                <defs>
                  <linearGradient id="barVariacao" x1="0" y1="0" x2="0" y2="1">
                ...
                <Bar
                  dataKey="variacao"
                  fill="url(#barVariacao)"
                  name="Variação do Mês"
                  radius={[6, 6, 0, 0]}
